    ...
    def generate(
        ...
        **model_kwargs
        ...
        if input_ids is None:
            ...
            assert input_ids.dim() == 2 or 'fuse_num' in model_kwargs, "Input prompt should be of shape (batch_size, sequence length)."
        ...
        if self.config.is_encoder_decoder:
            ...
            if 'encoder_outputs' not in model_kwargs:
                ...
                if 'fuse_num' in model_kwargs :
                    hidden_state_list = list()
                    for i in range(min(model_kwargs['fuse_num'], input_ids.shape[1])):
                        encoder_outputs: ModelOutput = encoder(input_ids[:,i,:], attention_mask=attention_mask[:,i,:], return_dict=True)
                        hidden_state_list.append(encoder_outputs.last_hidden_state)
                    encoder_outputs.last_hidden_state = torch.cat(hidden_state_list, dim=1)
                    attention_mask = attention_mask.reshape((batch_size, -1))
                    input_ids = input_ids.reshape((input_ids.shape[0], -1))
                elif 'event_ids' in model_kwargs:
                    encoder_outputs = encoder(
                        input_ids=input_ids,
                        attention_mask=attention_mask,
                        return_dict=True,
                        event_ids=model_kwargs['event_ids'],
                    )
                else:
                    encoder_outputs: ModelOutput = encoder(input_ids, attention_mask=attention_mask, return_dict=True)      
            else:
                encoder_outputs = BaseModelOutput(last_hidden_state=model_kwargs['encoder_outputs'][0], hidden_states=None, attentions=None)
            ##############################################

        # Expand input ids if num_beams > 1 or num_return_sequences > 1
        if num_return_sequences > 1 or num_beams > 1:
            #
            # Arthur's time testing code
            #
            # return None            
        ...
        if self.config.is_encoder_decoder:
            ##############################################
            if type(decoder_start_token_id) == int:
                ...
                input_ids = decoder_start_token_id.unsqueeze(0).repeat(effective_batch_size * num_beams, 1)
