...
#         self.hparams.git_sha = get_git_info()["repo_sha"]
        ...
        ##################
        # self.dataset_class = (Seq2SeqDatasetForFID)
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        outputs = self(src_ids, attention_mask=src_mask, decoder_input_ids=decoder_input_ids, use_cache=False, 
                        freeze_encoder=self.hparams.freeze_encoder, event_ids=batch['event_ids'] if 'event_ids' in batch else None)
        '''
        toc = time.time()
        self.timer_sum += (toc - tic)
        self.timer_count += 1
        if self.timer_count == 1000:
            print("TIMER FOR 1000: ", self.timer_sum)
            print("TIMER AVG 1000: ", self.timer_sum/1000)
        if self.timer_count == 3000:
            print("TIMER FOR 3000: ", self.timer_sum)
            print("TIMER AVG 3000: ", self.timer_sum/3000)
        '''
