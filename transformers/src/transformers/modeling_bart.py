...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        ############################ 
        # THIS PART OF CODE IS NOT FOR COMPATIBILITY WITH LATEST HUGGINGFACE TRANSFORMERS
        ############################ 
        self.embed_events = nn.Embedding(
                config.max_position_embeddings,
                embed_dim,
                0,  # 0 is non-event tag
            )
        if hasattr(config, 'etc_enabled') and config.etc_enabled:
            self.layers = nn.ModuleList([EncoderLayerWithETC(config) for _ in range(config.encoder_layers)])
        else:
            self.layers = nn.ModuleList([EncoderLayer(config) for _ in range(config.encoder_layers)])
        # self.layers = nn.ModuleList([EncoderLayer(config) for _ in range(config.encoder_layers)])
        self.embed_loss_func = nn.L1Loss()
        ############################ 
